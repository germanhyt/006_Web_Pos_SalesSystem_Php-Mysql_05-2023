...
class Productos extends Controllers
{
    ...
    public function insertar()
    {
        ...
        $precio = $_POST['precio'];
        $insert = $this->model->insertarProductos($codigo, $nombre,$categoria, $precio);
        if ($insert > 0) {
            $alert = 'registrado';
    ...
    public function actualizar()
    {
        ...
        $precio = $_POST['precio'];
        $actualizar = $this->model->actualizarProductos($codigo, $nombre,$categoria, $cantidad, $precio, $id);
        if ($actualizar == 1) {
            $alert =  'modificado';
        } else {
            $alert = 'error';
